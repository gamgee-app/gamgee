...
export const usePlex = (plexIp: string, plexToken: string) => {
    ...
    const ratingKey = session?.ratingKey ? parseInt(session.ratingKey) : undefined;
    const { metadata, error: metadataError } = usePlexMetadata(plexApi, ratingKey);

    const metadataGuidImdb = metadata?.guids?.find((guid) => guid.id.includes("imdb://"));
    ...
    useEffect(() => {
        if (playerState === "playing" && lastPlayerState !== playerState && viewOffset !== undefined) {
    ...
    const estimatedPlayTime = playerState === "playing" ? lastViewOffset + timeSinceLastUpdate : lastViewOffset;

    return {imdbId, estimatedPlayTime, playerState, error: sessionError ?? metadataError };
};
